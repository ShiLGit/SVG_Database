...
SVGimage* JSONtoSVG(const char* svgString){
  ...
  char buffJSON[1000] = "", buffTitle[1000] = "", buffDesc[1000] = "";
  ...
  char* tok;
  tok = strtok(buffJSON, ",");
  while(tok != NULL && strstr(buffJSON, ",") != NULL){
    printf("\ntoken: [%s]", tok);
  }
  //parse stoopid attr 
  sscanf("|aturday|", "|%s|", buffDesc);
  printf("\nbufTitle = '%s'\nbufdesc = '%s\n", buffTitle, buffDesc);
  strncpy(toReturn->title, buffTitle, 255);
  strncpy(toReturn->description, buffDesc, 255);
  return toReturn;
}
